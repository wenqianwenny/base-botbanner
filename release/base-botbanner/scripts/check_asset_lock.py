...
NAMED_ENTITY_MANIFEST_SECTION_RE = re.compile(
    r"^##\s+Named Entity Asset Manifest\s*$"
    r"(?P<body>.*?)"
    r"(?=^##\s+|\Z)",
    re.MULTILINE | re.DOTALL,
)
...
BACKTICK_LOCAL_ASSET_RE = re.compile(
    r"`(?P<path>(?:figma-refs|output/assets|assets)/[^`]+?\.(?:png|jpg|jpeg|gif|webp|svg))`",
    re.IGNORECASE,
)
...
AVATAR_CLASS_RE = re.compile(
    r"(?:^|[-_\\s])(?:avatar|member|assignee|person|user)(?:[-_\\s]|$)",
    re.IGNORECASE,
)
LOW_PRIORITY_AVATAR_RE = re.compile(r"(?:background|bg|ghost|placeholder|skeleton|low-priority)", re.IGNORECASE)
...
def extract_manifest_paths(brief_path: Path) -> tuple[set[str], str]:
    ...
    paths.update(
        normalize_manifest_path(item.group("path"))
        for item in BACKTICK_LOCAL_ASSET_RE.finditer(body)
    )
    return paths, body
...
def parse_named_entity_manifest(brief_path: Path) -> dict[str, dict[str, str]]:
    if not brief_path.exists():
        raise FileNotFoundError(f"Brief file not found: {brief_path}")
    markdown = brief_path.read_text(encoding="utf-8")
    match = NAMED_ENTITY_MANIFEST_SECTION_RE.search(markdown)
    if not match:
        return {}
    body = match.group("body")
    ...
    for raw_line in body.splitlines():
        ...
        if current is None:
            current = {}
        current[key] = normalize_manifest_path(value) if key in {"avatar", "asset"} else value
    if current and "name" in current:
        entities[current["name"]] = current
    return entities
...
def validate_avatar_assets(html_path: Path, skill_root: Path, locked_paths: set[str]) -> list[str]:
    html = html_path.read_text(encoding="utf-8")
    errors: list[str] = []
    ...
    for match in ANY_TAG_RE.finditer(html):
        ...
        if not AVATAR_CLASS_RE.search(class_value):
            continue
        if LOW_PRIORITY_AVATAR_RE.search(class_value):
            continue

        raw_path = attrs.get("src") or attrs.get("href") or attrs.get("data-asset") or ""
        if tag not in {"img", "image", "source"} or not raw_path:
            errors.append(
                "Visible avatar-like element must use an <img> asset from figma-refs/components/avatar/ "
                "(for grey generic avatars use `[D] Avatar_Person_grey.svg`), not a CSS-drawn circle: "
                f"class={class_value!r}"
            )
            continue

        normalized = normalize_rel(raw_path, html_path.parent, skill_root) if not raw_path.startswith("data:") and not is_external(raw_path) else raw_path
        if not normalized.startswith("figma-refs/components/avatar/"):
            errors.append(f"Avatar asset must come from figma-refs/components/avatar/: {normalized}")
        if normalized not in locked_paths:
            errors.append(f"Avatar asset is referenced but not locked in Asset Lock Manifest: {normalized}")
        if normalized.startswith("figma-refs/components/avatar/") and not (skill_root / normalized).exists():
            errors.append(f"Avatar asset does not exist: {normalized}")

    return errors


def validate_named_entity_assets(html_path: Path, brief_path: Path, skill_root: Path, locked_paths: set[str]) -> list[str]:
    html = html_path.read_text(encoding="utf-8")
    entity_locks = parse_named_entity_manifest(brief_path)
    errors: list[str] = []
    if not entity_locks:
        return errors

    for name, lock in entity_locks.items():
        avatar = lock.get("avatar") or lock.get("asset")
        asset_source = lock.get("asset_source")
        if not avatar:
            errors.append(f"Named entity {name!r} is missing avatar=... in Named Entity Asset Manifest.")
            continue
        if asset_source not in {"existing-avatar-file", "user-provided-avatar"}:
            errors.append(
                f"Named entity {name!r} must declare asset-source=existing-avatar-file or asset-source=user-provided-avatar."
            )
        if avatar not in locked_paths:
            errors.append(f"Named entity avatar is not locked in Asset Lock Manifest: {name} -> {avatar}")
        if not (skill_root / avatar).exists():
            errors.append(f"Named entity avatar asset does not exist: {name} -> {avatar}")
        if not avatar.startswith("figma-refs/components/avatar/"):
            errors.append(f"Named entity avatar must come from figma-refs/components/avatar/: {name} -> {avatar}")

    for match in ANY_TAG_RE.finditer(html):
        tag = match.group("tag").lower()
        attrs = parse_attrs(match.group("attrs"))
        name = attrs.get("data-entity-name")
        if not name:
            continue
        lock = entity_locks.get(name)
        if not lock:
            errors.append(f"HTML uses data-entity-name={name!r} but brief has no Named Entity Asset Manifest entry.")
            continue
        expected = lock.get("avatar") or lock.get("asset")
        raw_path = attrs.get("src") or attrs.get("href") or attrs.get("data-asset") or ""
        if tag not in {"img", "image", "source"} or not raw_path:
            errors.append(f"Named entity avatar for {name!r} must be an image tag with src/href/data-asset.")
            continue
        normalized = normalize_rel(raw_path, html_path.parent, skill_root) if not raw_path.startswith("data:") and not is_external(raw_path) else raw_path
        if expected and normalized != expected:
            errors.append(f"Named entity avatar mismatch for {name}: {normalized}, expected {expected}")
...
def validate(html_path: Path, brief_path: Path, skill_root: Path) -> list[str]:
    ...
    errors.extend(validate_avatar_assets(html_path, skill_root, locked_paths))
    errors.extend(validate_named_entity_assets(html_path, brief_path, skill_root, locked_paths))
