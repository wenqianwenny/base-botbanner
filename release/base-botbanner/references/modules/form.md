...
Panel rules:
- The polish result panel is a foreground floating result panel. Preserve the source panel radius, elevation intent, border, header icon, title, footer action styles, and internal padding from Figma; map the shadow to the global preset token.
- Default panel radius is `16px` only when the source radius is unavailable, following the global `.floating-panel` rule.
...
/* ui-check content-density selector=.polish-panel content-selector=.polish-panel-content top=24 right=24 bottom=24 left=24 */
