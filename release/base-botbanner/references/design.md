...
- use it to indicate the key interaction target in the foreground panel, not as decoration
...
- for an input target, prefer pointing to the right edge of the input field or send button area, while keeping the cursor body outside the input
...
- reposition into nearby whitespace; rotate only slightly when needed and record the reason in the brief
- keep at least `8px` clearance from product icons, readable text, selected values, buttons, input values, and core controls; keep the product evidence visible
- prefer placing the cursor beside the target, near a card edge, or in whitespace
...
- If simplification is required, remove details instead of redesigning components.
- Preserve rather than redesign when unsure.
- Allowed changes: abstraction, simplification, removing non-essential information, and replacing secondary content with skeletons.
- Not allowed: beautifying product UI, inventing new button styles, changing dropdown style, changing semantic colors, moving action positions, changing spacing hierarchy, changing component variants, changing typography hierarchy, or replacing real components with generic UI.
- Forbidden examples: changing the send button style, changing the position of `另存为新视图`, changing the visual weight of `所有`, changing the style of generated-condition feedback text, or inventing new interaction patterns.
...
Rules:
- Use at least 3 different line widths in repeated abstract rows to suggest real copy variety
- Do not use one shared percentage width for every row in a repeated list/table/card group
...
- Avatars: every visible avatar/member/user/person/assignee visual must use an `<img>` asset from `figma-refs/components/avatar/`.
...
- atmospheric
- Apple-like
- physically above the secondary UI, similar to ChatGPT input cards or Apple floating surfaces
...
- opacity greater than `25%`
- neumorphism emboss effects

Use:
- same-hue shadows
- low saturation
- cool gray-blue tones
- large blur with low opacity
- multiple shadow layers
- hierarchy through shadow spread, not borders

Do not invent shadow values. Only use predefined shadow tokens.
...
Purpose: primary floating panels should visibly hover above the secondary UI with obvious but soft depth.
...
Rules:
- Primary panel shadow must be stronger than the secondary panel shadow.
- Shadow hue follows the background hue; use the predefined token unless a future token is explicitly added to this file.
- Do not replace this with black, gray-black, one-layer, or custom hand-tuned shadows.
...
- Hard rule: any visible avatar must come from this folder. This includes real people, named users, generic members, grey placeholders, IM avatars, table assignees, mentions, stacked avatars, and background member context.
...
- If the user has provided or updated avatar assets, place them under `figma-refs/components/avatar/` first, then use that exact file.
- Before deriving a new named avatar from a screenshot, inspect `figma-refs/components/avatar/` and reuse the existing matching avatar file. Existing avatar-folder assets are the source of truth.
- Before coding, lock exact avatar filenames in the Asset Lock Manifest.
- Every visible avatar must be an `<img>` using a locked asset from `figma-refs/components/avatar/`. CSS-drawn faces, initials, gradient circles, emoji-style heads, plain circles, cropped screenshots, or random web photos are not allowed.
- If several real-person avatars appear, use distinct preset `Avatar-image-*` files where possible. For low-priority background avatars, reuse `[D] Avatar_Person_grey.svg` or another locked avatar-folder asset; do not draw a placeholder circle.

## Named Entity Asset Binding

Named users are identity-bound entities.

Rules:
- If a user avatar asset is provided, reuse the exact same avatar-folder asset for that user.
- Identity consistency is mandatory across all UI layers: table, floating panel, filter, comments, chat, mention, and member tags.
- All image elements for a named user must carry `data-entity-name="<name>"`.
- Add a `Named Entity Asset Manifest` entry in the brief for every named user with a provided avatar.

Example:

```markdown
## Named Entity Asset Manifest
- name: 李天天
  avatar: figma-refs/components/avatar/Avatar-image.svg
  asset-source: existing-avatar-file
```

Forbidden:
- different faces for the same user
- generated, stock, generic, or approximate replacement avatars
- substituting another `Avatar-image-*` when a source avatar asset exists
- creating a cropped PNG from screenshot when the avatar already exists under `figma-refs/components/avatar/`
- drawing any visible avatar as CSS or a plain shape

Use source asset as the single source of truth. If an avatar asset exists, never regenerate that avatar.
